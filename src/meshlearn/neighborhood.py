...
def mesh_neighborhoods_coords(neighborhoods, tmesh, num_neighbors=None, do_center=True):
    """
    ...
    for central_vertex, neighbors in neighborhoods.items(): # TODO: Check whether central vertex is part of the neighborhood, add it if not.
        if num_neighbors is None:
            neigh_coords = tmesh.vertices[neighborhoods[vert_idx], :]
        else:
            neigh_coords = tmesh.vertices[neighborhoods[vert_idx][0:(num_neighbors-1)], :]
